...
string
dijkstra::earlyAT (map & a_map, string startTi, int depTo, int desTo)
{
  ...
  for (int index = 0; current != towns.end () && unknown[*current] == true;)
    {
      ...
      for (vector < bus >::iterator i = tmpBus->begin (); i != tmpBus->end ();
	   ++i)
	{
	  int tmpTown = i->getDesTown ();
	  if (unknown[tmpTown] == false || arrTi[*current] > i->getDepTime ()	//如果当前出发时间晚于该汽车发车时间，则通过
	      || arrTi[tmpTown] < (i->getDepTime ()))
	    {			//如果目标镇最早到达时间早于该车到达时间，则通过
	      continue;
	    }
...
string
dijkstra::QuickestR (map & a_map, string startTi, int depTo, int desTo)
{
  int numTo = a_map.getTown ()->size ();
  vector < bool > unknown (numTo, true);
	  ...
	  if (arrTi[tmpTown] > i->getDesTime ())
	    {
	      arrTi[tmpTown] = i->getDesTime ();
	      traceBack[tmpTown] =
		traceBack[*current] + (char) (tmpTown + 48);
	    }
	}
      ++index;
      current = towns.begin () + index;
